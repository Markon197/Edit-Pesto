...
RULES
1. Fix only genuine errors: typos, misspellings, subject-verb agreement, punctuation mistakes, a wrong word (e.g. "there" for "their"), and phrasing that is actually grammatically wrong or clearly reads incorrectly - not just phrasing you would personally word differently.
2. Spacing errors are a distinct, high-priority category - do not rely on a normal read-through to catch these, since they're easy to skim past. Deliberately scan the raw text character-by-character for: two spaces in a row anywhere (including after a full stop), a missing space where two words have been run together with no space between them (e.g. "wordanother", "bothannounced"), and a missing space after punctuation (e.g. a comma or full stop directly followed by the next word). Every instance must be caught - treat this as a mandatory separate pass, not a byproduct of reading for meaning.
3. Do not paraphrase, restructure sentences, change tone, shorten, or "improve" writing that is already correct. When in doubt, leave it alone.
4. Apply the InsuranceERM style guide below wherever it's relevant (abbreviations, currency formatting, capitalisation, quote marks, numbers, dates, tense, etc.) as part of what counts as "correct" for this publication.
5. The input is an HTML fragment. Every HTML tag - links (and their href), bold, italics, paragraphs, lists - is structural and must be preserved exactly. Never add, remove, or change a tag or an href. Only the visible text may be edited.
6. For every change, wrap the exact original text in <del> and your replacement in <ins>, placed immediately next to each other, e.g. "sever<del>ve</del><ins>e</ins> storms". Keep each <del>/<ins> pair as small and localised as possible - never wrap a whole sentence for a one-word fix. Everything you did not change must appear in the output completely unmodified, including all surrounding tags and whitespace.
7. If the article has no errors at all, return annotated_html identical to the input with no <del>/<ins> tags.
8. Also extract, from the article text: five alternative headlines, every company mentioned (deduplicated, full name as it first appears), and every named person mentioned (deduplicated, full name).
